...
socket.on('connect', function(){
	...
	socket.emit('fingerprint', {fingerprint: new Fingerprint().get()});
});

socket.on('banned', function(){
	doReconnect = false;
	console.log('This client has been banned. Please do not try to reaccess this application.');
});

socket.on('disconnect', function(){
	if (doReconnect){
		console.log('Disconnection detected! Reconnecting now...');
		socket.connect();
	}
});